...
const localDevicesPath = require.resolve('local-devices');
let localDevices = require('local-devices');
const findLocalDevices = (options) => {
    return localDevices(options).catch((err) => {
        delete require.cache[localDevicesPath];
        localDevices = require('local-devices');
        return localDevices(options);
    });
};
...
const init = function() {
    ...
    const net = new NetworkObserver(this.interval, this.range, this.skipPing)
...
class NetworkObserver extends EventEmitter {
    constructor(interval, range, skipPing) {
        super();
        this.pingOptions = {
            timeout: 2000
        };
        ...
        this.range = range
        this.skipPing = skipPing !== undefined ? skipPing : true // Default to true
        this.cachedDevices = [];
        this.tick();
    }
